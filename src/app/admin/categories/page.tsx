...
export default function CategoriesAdmin() {
  ...
  const openEditModal = (category: WorkshopCategory) => {
  ...
  const handleDeleteCategory = async (category: WorkshopCategory) => {
  ...
  const handleToggleActive = async (category: WorkshopCategory) => {
    try {
      const { error } = await supabase
        .from("workshop_categories")
        // @ts-expect-error - Supabase types not inferring correctly
  ...
  const handleMoveCategory = async (category: WorkshopCategory, direction: "up" | "down") => {
    ...
    try {
      // @ts-expect-error - Supabase types not inferring correctly
      await supabase.from("workshop_categories").update({ order_position: newIndex }).eq("id", category.id);
      // @ts-expect-error - Supabase types not inferring correctly
      await supabase.from("workshop_categories").update({ order_position: currentIndex }).eq("id", otherCategory.id);
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm text-purple-900">
                  <strong>As categorias são usadas para organizar e filtrar oficinas na página pública.</strong>
                  Ao criar/editar uma oficina, você pode associá-la a uma categoria.
                </p>
                <Link
