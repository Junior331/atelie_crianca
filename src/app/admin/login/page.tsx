...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.session) {
        document.cookie = `supabase-auth-token=${data.session.access_token}; path=/; max-age=3600`
        router.push('/admin')
      }
    } catch (err: any) {
      ...
      <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-lg shadow-md">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Ateliê da Criança - Painel Administrativo
          </p>
        </div>
        <form className="mt-8 space-y-6" onSubmit={handleLogin}>
          ...
          <div>
            <button
              ...
              className="group relative w-full flex items-center justify-center gap-2 py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-pink-600 hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
